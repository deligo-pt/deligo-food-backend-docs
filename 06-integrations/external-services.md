...
**Where used**: `src/app/utils/storage.ts` — `resolveUploadedImageUrl` (thin `req.file` → URL wrapper around `uploadAndOptimizeImage`; returns `null` if no file was attached), `uploadAndOptimizeImage` (resizes to 800×600 WebP via `sharp`, uploads PDFs raw), `deleteSingleImageFromRustFS`, `deleteMultipleImagesFromRustFS`, `uploadLocalFileToRustFS`. Called from any module accepting file uploads (`Product`, `Vendor`, `Agreement`, `Profile`, `Upload`, `Admin`, `Category` (business/product/cuisine), `Delivery-Partner`, `Payout`, `Sponsorships`, etc.) — since multer is configured with `memoryStorage()` (`src/app/config/multer.config.ts`), controllers must go through this helper rather than reading `file.path`, which memory storage never populates.
...
**Filterable attributes** (`FOOD_INDEX_SETTINGS.filterableAttributes`, `meilisearch.constant.ts`, applied via `updateSettings` in `meilisearch.init.ts` on startup): `_geo`, `cuisine`, `categoryIds`, `isHalal`, `price`, `restaurantId`, `isAvailable`. `categoryIds` was added 2026-08-24; since 2026-08-29 it holds just the product's single (vendor-owned) `category` id — still an array (length 1) so the attribute and the `categoryIds = "<id>"` filter stay stable (`buildFoodSearchDocument`, `meilisearch.utils.ts`). `GET /search?category=<id>` (`meilisearch.service.ts`) filters on it. Existing indexed docs need a reindex (`npm run meili:migrate`) after the change. See [`../03-modules/product-and-catalog.md`](../03-modules/product-and-catalog.md).
