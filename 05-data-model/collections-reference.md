...
Fields: `productId` (unique), `vendorId` (→ Vendor), `sku` (unique), localized `name`/`description`, `isDeleted`, `isApproved`, `approvedBy` (→ Admin), `category` (→ ProductCategory), `subCategory`, `brand`, `variations[]` (embedded, each option with its own `sku`/`price`/`stock`), `addonGroups[]` (→ AddonGroup, many-to-many), `pricing: {taxId → Tax, taxRate, discount, discountType}`, `stock: {quantity, availabilityStatus, hasVariations}` (undefined for RESTAURANT-type vendors — see [`../03-modules/product-and-catalog.md`](../03-modules/product-and-catalog.md)), `images[]`, `rating`, `meta: {isFeatured, status: ACTIVE|INACTIVE|DELETED, origin}`.
Indexes: unique on `productId`, `sku` (schema-level).
...
### BusinessCategory / ProductCategory / Cuisine — `src/app/modules/Category/category.model.ts` (one file, three collections)
...
- `ProductCategory`: `name`, `slug` (unique), `businessCategoryId` (→ BusinessCategory, required), `isActive`, `isDeleted`. Index `{isActive:1, isDeleted:1}`, unique on `name.en`.
...
Fields: `establishmentName`, `email` (unique), `contactNumber`, `nif`, `isEmailVerified`, `emailVerifiedAt`, `draftPdfPath`/`agentSignaturePath`/`establishmentSignaturePath`/`signedPdfPath`, `status` (`PENDING_VERIFICATION → VERIFIED → DRAFT → SIGNED → EMAILED`), `signedAt`/`emailedAt`, `createdBy` (→ Admin).
Soft delete: none.
